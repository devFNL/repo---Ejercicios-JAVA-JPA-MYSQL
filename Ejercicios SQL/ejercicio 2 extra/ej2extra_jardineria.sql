...
select c.nombre_cliente, CONCAT(e.nombre,' ',apellido1,' ',apellido2) as representante
from cliente c
inner join empleado e 
on c.codigo_empleado_rep_ventas = e.codigo_empleado;
-- 2. Muestra el nombre de los clientes que hayan realizado pagos junto con el nombre de sus representantes de ventas.
select c.nombre_cliente, e.nombre as representante
from cliente c
inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado
inner join pago p on c.codigo_cliente = p.codigo_cliente;
-- 3. Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de sus representantes de ventas.
select c.nombre_cliente, CONCAT(e.nombre,' ',e.apellido1,' ',e.apellido2) as representante
from cliente c
inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado
inner join pago p on c.codigo_cliente != p.codigo_cliente;
-- 4. Devuelve el nombre de los clientes que han hecho pagos y el nombre de sus representantes junto con la ciudad de la oficina a la que pertenece el representante.
select c.nombre_cliente, CONCAT(e.nombre,' ',apellido1,' ',apellido2) as representante, o.ciudad
from cliente c
inner join empleado e on c.codigo_empleado_rep_ventas = e.codigo_empleado
...
select e.nombre as empleado, CONCAT(i.nombre, ' ', i.apellido1, ' ', i.apellido2) as jefe
from empleado e
inner join empleado i on e.codigo_jefe = i.codigo_empleado;
...
select pro.gama
from producto pro
inner join detalle_pedido de on pro.codigo_producto = de.codigo_producto
inner join pedido pe on pe.codigo_pedido = de.codigo_pedido
...
select *
from producto pro
left join detalle_pedido de on pro.codigo_producto = de.codigo_producto
...
left join cliente c on c.codigo_empleado_rep_ventas = e.codigo_empleado
left join pedido pe on pe.codigo_cliente = c.codigo_cliente
left join detalle_pedido de on pe.codigo_pedido = de.codigo_pedido
left join producto pro on de.codigo_producto = pro.codigo_producto
...
select count(codigo_pedido), estado
from pedido
group by estado;
...
select count(c.codigo_cliente), CONCAT(e.nombre, ' ', e.apellido1, ' ', e.apellido2) as nombre_representante
...
select p.codigo_pedido, count(distinct de.codigo_producto) as productos_diferentes
from pedido p, detalle_pedido de
where p.codigo_cliente = de.codigo_pedido
group by p.codigo_pedido;

select p.codigo_pedido, count(distinct de.codigo_producto) as productos_diferentes
from pedido p
inner join detalle_pedido de on p.codigo_cliente = de.codigo_pedido
group by p.codigo_pedido;
-- 13. Calcula la suma de la cantidad total de todos los productos que aparecen en cada uno de los pedidos.
select p.codigo_pedido, sum(de.codigo_producto) as total_productos
from pedido p, detalle_pedido de
where p.codigo_pedido = de.codigo_pedido
...
unidades vendidas. */
select p.codigo_producto, p.nombre, sum(de.cantidad) as total_vendidos
from detalle_pedido de
inner join producto p on de.codigo_producto = p.codigo_producto
...
IVA)*/
select p.nombre, sum(de.cantidad), sum(de.precio_unidad * de.cantidad) as total, sum(de.precio_unidad * de.cantidad * 1.21) as total_mas_iva
from producto p
inner join detalle_pedido de on p.codigo_producto = de.codigo_producto
group by p.codigo_producto
having sum(de.cantidad * de.precio_unidad) > 3000
order by total desc;
